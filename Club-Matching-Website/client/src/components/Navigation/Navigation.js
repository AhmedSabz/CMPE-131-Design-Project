...
function Navigation({ isUserLoggedIn, isClubLoggedIn, isAdminLoggedIn, isAccountLoggedIn ,onLogout }) {
    ...
    return (
        <nav className="navbar">
            ...
            <ul>
                ...
                {(isUserLoggedIn) && <li><button onClick={handleDashboardButtonClick}>Account Dashboard</button></li>}
                ...
                {(isUserLoggedIn) && <li><button onClick={handleSurveyButtonClick}>Club Matching Survey</button></li>}
                {(isAdminLoggedIn && isAccountLoggedIn && !isClubLoggedIn && !isUserLoggedIn) && <li><button onClick={handleDatabase}>Database</button></li>}
                {(isAccountLoggedIn) && <li><button onClick={handleLogout}>Log Out</button></li>}
            </ul>
        </nav>
    );
};
